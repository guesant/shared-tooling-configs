...
Quick reference for Keycloak, a tool or engineering platform mentioned in the `guesant/hl-infrastructure` documentation.

## Repository mapping

- Catalog id: `keycloak`
- Configuration namespace: `tools/keycloak/`
- Discovery source: `guesant/hl-infrastructure/docs/`
- Current phase: inventory and reference

## Entry point

This project does not assume a universal standalone CLI for Keycloak. Inspect it through the package manager, deployment manifest, Helm chart, provider, service API, or the control plane that owns it.
