...
Quick reference for Redis, a tool or engineering platform mentioned in the `guesant/hl-infrastructure` documentation.

## Repository mapping

- Catalog id: `redis`
- Configuration namespace: `tools/redis/`
- Discovery source: `guesant/hl-infrastructure/docs/`
- Current phase: inventory and reference

## Entry point

This project does not assume a universal standalone CLI for Redis. Inspect it through the package manager, deployment manifest, Helm chart, provider, service API, or the control plane that owns it.
